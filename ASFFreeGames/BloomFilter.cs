...
namespace BloomFilter {
	...
	public class Filter<T> {
		...
		public byte[] ToArray() {
			var res = new byte[(_hashBits.Count / 8) + (_hashBits.Count % 8 == 0 ? 0 : 1)];

			for (int i = 0; i < _hashBits.Count; i++) {
				res[i / 8] |= (byte) ((_hashBits[i] ? 1 : 0) << (i % 8));
			}
		...
		public int Populate(ReadOnlySpan<byte> span) {
			var leftOver = (_hashBits.Count % 8 == 0 ? 0 : 1);
			var c = 0;

			if (span.Length != (_hashBits.Count / 8) + leftOver) {
