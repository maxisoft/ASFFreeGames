...
public partial class LoggerFilter {
	...
	private readonly ConcurrentDictionary<BotName, LinkedList<Func<LogEventInfo, bool>>> Filters =
		new();
	...
	public IDisposable DisableLogging(Func<LogEventInfo, bool> filter, [NotNull] Bot bot) {
		...
		lock (Filters) {
			...
			foreach (
				LoggingRule loggingRule in config.LoggingRules.Where(loggingRule =>
					!loggingRule.Filters.Any(f => ReferenceEquals(f, MethodFilter))
				)
			) {
	...
	/// <param name="bot">The bot instance whose logging should be disabled.</param>
	/// <returns>A disposable object that can be used to re-enable logging when disposed.</returns>
	public IDisposable DisableLoggingForAddLicenseCommonErrors(
		Func<LogEventInfo, bool> filter,
		[NotNull] Bot bot
	) {
		bool filter2(LogEventInfo info) =>
			(info.Level == LogLevel.Debug)
			&& filter(info)
			&& AddLicenseCommonErrorsRegex().IsMatch(info.Message);
	...
	[GeneratedRegex(
		@"^.*?InternalRequest(?>\s*)\(\w*?\)(?>\s*)(?:(?:InternalServerError)|(?:Forbidden)).*?$",
	...
	private FilterResult FilterLogEvent(LogEventInfo eventInfo) {
		...
		if (
			Filters.TryGetValue(
				bot?.BotName ?? eventInfo.LoggerName ?? "",
				out LinkedList<Func<LogEventInfo, bool>>? filters
			)
		) {
			return filters.Any(func => func(eventInfo))
	...
	private sealed class LoggerRemoveFilterDisposable(LinkedListNode<Func<LogEventInfo, bool>> node)
		: IDisposable {
		public void Dispose() => node.List?.Remove(node);
	}

	// A class that implements a custom filter that invokes a method
	private class MarkedWhenMethodFilter(Func<LogEventInfo, FilterResult> filterMethod)
		: WhenMethodFilter(filterMethod);
}
